...
/**
 * Test class validating the complete low-risk user onboarding flow in the Pilla application.
...
@Feature("Low-Risk Onboarding Flow")
@Story("As a new user, I want to register, define my profile as low-risk, select a product, and complete the onboarding process.")
public class LowRiskOnboardingFlowTest extends BaseTest {
    ...
    private ThreeALowRiskPortfolioPage threeALowRiskPortfolioPage;
    ...
    @Test(priority = 5, dependsOnMethods = "testFillPersonalAndInvestmentProfile", description = "Select the 'Low Risk' profile and a corresponding product.")
    @Description("Overrides the determined profile to select 'Low Risk', then chooses the 'Account' solution product.")
    public void testLowRiskProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Low Risk'", () -> {
                    investmentProfileSelectionPage = investmentProfilePage.clickSelectDifferentInvestmentProfileButton();
                    investmentProfileSelectionPage.clickLowRiskProfile();
                    Thread.sleep(2000);
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("Your choice: Low risk"),
                            "Text 'Your choice: Low risk' not found on screen."
                    );
                }
        );

        Allure.step(
                "Confirm the low-risk profile and select the 'Account' product", () -> {
                    threeALowRiskPortfolioPage = investmentProfileSelectionPage.clickThreeALowRiskConfirmButton();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The following range of products is available to you"),
                            "Product range text not found on page."
                    );

//                    WebDriverWait wait = new WebDriverWait((WebDriver) driver, Duration.ofSeconds(2000));
//                    WebElement lowRiskSolution;
//                    WebElement confirmButton;
//                    WebElement selectContinueButton;
//                    String lowRiskSolutionLocator = getLocators().get("product-selection-lowrisk-solution");
...
//
//                    String confirmButtonLocator = getLocators().get("product-selection-confirm");
//                    confirmButton = wait.until(ExpectedConditions.visibilityOfElementLocated(AppiumBy.xpath(confirmButtonLocator)));
//                    confirmButton.click();
//
//                    String selectStrategyButtonLocator = getLocators().get("product-selection-strategy-button");
...
//                    selectContinueButton.click();
                    ...
                    transferingYourCapitalPage = threeALowRiskPortfolioPage
                            .clickThreeALowRiskSolution()
                            .clickConfirmButton()
    ...
    @Test(priority = 6, dependsOnMethods = "testLowRiskProfileAndProductSelection", description = "Complete final details, upload documents, and agree to terms.")
