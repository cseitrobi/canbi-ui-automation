...
@Feature("Balanced Onboarding Flow")
@Story("As a new user, I want to register, define my profile as Balanced, select a product, and complete the onboarding process.")
public class BalancedOnboardingFlowTest extends BaseTest {
    ...
    private BalancedProfilePage balancedProfilePage;
    ...
    @Test(priority = 5, dependsOnMethods = "testFillPersonalAndInvestmentProfile", description = "Select the 'Balanced' profile and a corresponding product.")
    ...
    public void testBalancedProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Balanced'", () -> {
                    investmentProfileSelectionPage = investmentProfilePage.clickSelectDifferentInvestmentProfileButton();
                    investmentProfileSelectionPage.clickBalancedProfile();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("Your risk profile corresponds to a balanced strategy"),
                            "Text 'Your risk profile corresponds to a balanced strategy' not found on screen."
                    );
                }
        );
        ...
        Allure.step(
                "Confirm the balanced-risk profile and select the 'Balanced' product", () -> {
                    balancedProfilePage = investmentProfileSelectionPage.clickBalancedConfirmButton();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The following range of products is available to you"),
                            "Product range text not found on page."
                    );

                    transferingYourCapitalPage = balancedProfilePage
                            .clickBalancedSolution()
                            .clickConfirmButton()
