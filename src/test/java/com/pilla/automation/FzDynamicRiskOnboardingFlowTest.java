...
@Feature("Dynamic Onboarding Flow for vested benefit solutions")
...
public class FzDynamicRiskOnboardingFlowTest extends BaseTest {
    ...
    public void testDynamicProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Dynamic'", () -> {
                    ...
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The dynamic risk profile corresponds to a strategy targeting higher price"),
                            "The dynamic risk profile corresponds to a strategy targeting higher price' not found on screen."
                    );
                }
        );
        ...
        Allure.step(
                "Confirm the Dynamic-risk profile, accept disclaimer, and go to product selection", () -> {
                    dynamicProfilePage = investmentProfileSelectionPage.clickDynamicConfirmButton();
                }
        );
    }

    @Test(priority = 6, dependsOnMethods = "testDynamicProfileAndProductSelection", description = "Select Disclaimer text checkbox and confim.")
    @Description("Provides necessary disclaimer text as higher risk is selected.")
    public void testDynamicPage() throws InterruptedException {
        Allure.step(
                "Confirm dynamic page", () -> {
                    ...
                    Assert.assertTrue(
                            ...
                            "Transfer-your-capital prompt not found."
                    );
                }
        );
    ...
    @Test(priority = 7, dependsOnMethods = "testDynamicPage", description = "Complete final details, upload documents, and agree to terms.")
