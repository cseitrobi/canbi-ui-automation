...
public class FzLowRiskOnboardingFlowTest extends BaseTest {
    ...
    private LowRiskPortfolioPage lowRiskPortfolioPage;
    ...
    @Description("Overrides the determined profile to select 'Low Risk', then chooses the corresponding solution product from the full selection screen.")
    public void testLowRiskProfileAndProductSelection() throws InterruptedException {
        ...
        Allure.step(
                "Confirm the low-risk profile and select the product", () -> {
                    lowRiskPortfolioPage = investmentProfileSelectionPage.clickLowRiskConfirmButton();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The following range of products is available to you"),
                            "Product range text not found on page."
                    );

                    transferingYourCapitalPage = lowRiskPortfolioPage
                            .clickLowRiskSolution()
                            .clickConfirmButton()
