...
public class FzCapitalRiskOnboardingFlowTest extends BaseTest {
    ...
    public void testBalancedProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Balanced'", () -> {
                    ...
                    investmentProfileSelectionPage.clickCapitalProfile();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("Your risk profile corresponds to a strategy where higher price and/or foreign exchange gains are significantly more heavily weighted than regular income"),
                            "Text 'Your risk profile corresponds to a Capital Gains strategy' not found on screen."
                    );
                }
        );

        Allure.step(
                "Confirm the capital-risk profile and select the 'Capital Gains' product", () -> {
                    capitalProfilePage = investmentProfileSelectionPage.clickCapitalConfirmButton();
                    Assert.assertTrue(
