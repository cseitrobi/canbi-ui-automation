...
@Feature("Growth Onboarding Flow")
@Story("As a new user, I want to register, define my profile as Growth, select a product, and complete the onboarding process.")
public class GrowthRiskOnboardingFlowTest extends BaseTest {
    ...
    private GrowthProfilePage growthProfilePage;
    ...
    @Test(priority = 5, dependsOnMethods = "testFillPersonalAndInvestmentProfile", description = "Select the 'Growth' profile and a corresponding product.")
    @Description("Overrides the determined profile to select 'Growth', then chooses the 'Growth' solution product.")
    public void testGrowthProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Growth'", () -> {
                    investmentProfileSelectionPage = investmentProfilePage.clickSelectDifferentInvestmentProfileButton();
                    investmentProfileSelectionPage.clickGrowthProfile();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("Your risk profile corresponds to a strategy where higher price"),
                            "Text 'Your risk profile corresponds to a strategy where higher price' not found on screen."
                    );
                }
        );

        Allure.step(
                "Confirm the growth-risk profile and select the 'Growth' product", () -> {
                    growthProfilePage = investmentProfileSelectionPage.clickGrowthConfirmButton();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The following range of products is available to you"),
                            "Product range text not found on page."
                    );

                    transferingYourCapitalPage = growthProfilePage
                            .clickGrowthSolution()
                            .clickConfirmButton()
    ...
    @Test(priority = 6, dependsOnMethods = "testGrowthProfileAndProductSelection", description = "Complete final details, upload documents, and agree to terms.")
