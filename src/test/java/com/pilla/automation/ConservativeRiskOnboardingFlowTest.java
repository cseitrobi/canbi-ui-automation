...
@Feature("Conservative Onboarding Flow")
@Story("As a new user, I want to register, define my profile as Conservative, select a product, and complete the onboarding process.")
public class ConservativeRiskOnboardingFlowTest extends BaseTest {
    ...
    private ConservativePortfolioPage conservativePortfolioPage;
    ...
    @Test(priority = 5, dependsOnMethods = "testFillPersonalAndInvestmentProfile", description = "Select the 'Conservative' profile and a corresponding product.")
    @Description("Overrides the determined profile to select 'Conservative', then chooses the 'Account' solution product.")
    public void testConservativeProfileAndProductSelection() throws InterruptedException {
        Allure.step(
                "Change the investment profile to 'Conservative'", () -> {
                    investmentProfileSelectionPage = investmentProfilePage.clickSelectDifferentInvestmentProfileButton();
                    investmentProfileSelectionPage.clickConservativeRiskProfile();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("Your risk profile corresponds to a conservative strategy"),
                            "Text 'Your risk profile corresponds to a conservative strategy' not found on screen."
                    );
                }
        );

        Allure.step(
                "Confirm the conservative-risk profile and select the 'Conservative' product", () -> {
                    conservativePortfolioPage = investmentProfileSelectionPage.clickConservativeConfirmButton();
                    Assert.assertTrue(
                            getDriver().getPageSource().contains("The following range of products is available to you"),
                            "Product range text not found on page."
                    );

                    transferingYourCapitalPage = conservativePortfolioPage
                            .clickConservativeSolution()
                            .clickConfirmButton()
    ...
    @Test(priority = 6, dependsOnMethods = "testConservativeProfileAndProductSelection", description = "Complete final details, upload documents, and agree to terms.")
